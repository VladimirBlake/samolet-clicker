        ...
        if (promocodeLeft?.documentId) {
          ...
          if (chat_id) {
            await bot.api.sendMessage(
              Number(chat_id),
              `Ваш уникальный промокод ${promocodeLeft.promocode_id} на скидку 8% на все квартиры от Самолет* во всех городах присутствия.

\*_Промокод не суммируется с другими акциями и предложениями. Период действия акции: 20.11.24 по 28.02.25. Не является публичной офертой._`,
              { parse_mode: "Markdown" }
            );
          }
        }
